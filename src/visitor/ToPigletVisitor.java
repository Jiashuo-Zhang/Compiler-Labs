...
public class ToPigletVisitor extends GJVoidDepthFirst<MType> {
	...
	public void visit(AllocationExpression n, MType argu) {
		...
		for (MMethod method : newclass.getAllMethods()) {
			document.write("HSTORE", temp1, offset, method.getPigletName());
			document.newline();
			offset += 4;
		}
		...
		for (MVar var : newclass.getAllVars()) {
			document.write("HSTORE", temp2, offset, 0);/// initialize
			document.newline();
			offset += 4;
		}
	...
	public void visit(MessageSend n, MType argu) {
		...
		if (method.getParamNum() <= 19) {
			...
			String param = paramsvisitor.document.sb.toString();
			param = param.trim();
			this.document.write(param);
			this.document.write(")");
		}
			...
			if (n.f1.present() == true) {
				for (Enumeration<Node> e = n.f1.elements(); e.hasMoreElements();) {
					if (cnt <= 18) {
						...
						String param = OneParamVisitor.document.sb.toString();
						param = param.trim();
						document.write("HSTORE", temp19, 4 * (cnt - 20), param);
						document.newline();
					}
					cnt++;
				...
				System.out.println("ERROR! The number of param is Wrong");
		} else {
			n.f0.accept(this, argu);
			n.f1.accept(this, argu);
		}
