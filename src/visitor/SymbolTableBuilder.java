...
public class SymbolTableBuilder extends GJDepthFirst<MType, MType> {
	...
	public MType visit(MainClass n, MType argu) {
		...
		try {
			nowClass.insertMethod(nowMethod);
		} catch (RedefinitionException e) {
			Abort.abort(e);
		}
	...
	public MType visit(VarDeclaration n, MType argu) {
		...
		MVar nowVar = new MVar((MIdentifier) argu, nowID.getName(), nowType.getType(), nowType.getRow(),
				nowType.getCol());
		if (argu.getType() == "class") {
			try {
				((MClass) argu).insertVar(nowVar);
			} catch (RedefinitionException e) {
				Abort.abort(e);
			}
		} else if (argu.getType() == "method") {
			try {
				((MMethod) argu).insertVar(nowVar);
			} catch (RedefinitionException e) {
				Abort.abort(e);
			}
		}
	...
	public MType visit(FormalParameter n, MType argu) {
		...
		MVar nowVar = new MVar((MIdentifier) argu, paramID.getName(), paramType.getType(), paramType.getRow(),
				paramType.getCol());
		try {
			((MMethod) argu).insertParam(nowVar);
		} catch (RedefinitionException e) {
			Abort.abort(e);
		}
